...
import com.example.bankingapp.Adapters.AccountAdapter;
import com.example.bankingapp.Model.Account;
import com.example.bankingapp.Model.Profile;
import com.example.bankingapp.Model.db.ApplicationDB;
...
public class AccountOverviewFragment extends Fragment {
    ...
    private final OnClickListener addAccountClickListener = new OnClickListener() {
        @Override
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container,
        ...
        Objects.requireNonNull(getActivity()).setTitle("Accounts");
    ...
    private void displayAccountDialog() {
        ...
        accountDialog = new Dialog(Objects.requireNonNull(getActivity()));
        accountDialog.setContentView(R.layout.account_dialog);

        Objects.requireNonNull(accountDialog.getWindow()).setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
    ...
    private void setValues() {
        ...
        userPreferences = Objects.requireNonNull(this.getActivity()).getSharedPreferences("LastProfileUsed", MODE_PRIVATE);
        gson = new Gson();
        ...
        if (userProfile.getAccounts().size() == 0) {
            ...
            txtDetailMessage.setVisibility(GONE);
            lstAccounts.setVisibility(GONE);
        } else {
            txtTitleMessage.setText("Select an Account to view Transactions");
            txtDetailMessage.setVisibility(VISIBLE);
            lstAccounts.setVisibility(VISIBLE);
        }
    ...
    private void viewAccount() {
        com.example.bankingapp.TransactionFragment transactionsFragment = new TransactionFragment();
